...
def test_paragraph_text_recognizer():
    ...
    for i, support_filename in enumerate(support_filenames):
        ...
        cer = _character_error_rate(
            support_data_by_file_id[support_filename.stem]["ground_truth_text"],
            predicted_text,
        )
        print(
...
def _test_paragraph_text_recognizer(
    image_filename: Path, expected_text: str, text_recognizer: ParagraphTextRecognizer
):
    """Test ParagraphTextRecognizer on 1 image."""
    predicted_text = text_recognizer.predict(image_filename)
    assert (
        predicted_text == expected_text
    ), f"predicted text does not match expected for {image_filename.name}"
